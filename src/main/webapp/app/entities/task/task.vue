<template>
  <div>
    <h2 id="page-heading" data-cy="TaskHeading">
      <span v-text="$t('synergyDogApp.task.home.title')" id="task-heading">Tasks</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('synergyDogApp.task.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'TaskCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-task">
            ...
            <span v-text="$t('synergyDogApp.task.home.createLabel')"> Create a new Task </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && tasks && tasks.length === 0">
      <span v-text="$t('synergyDogApp.task.home.notFound')">No tasks found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="tasks">
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('board.name')">
              <span v-text="$t('synergyDogApp.task.board')">Board</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'board.name'"></jhi-sort-indicator>
            </th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="task in tasks" :key="task.id" data-cy="entityTable">
            ...
            <td>{{ task.title }}</td>
            <td>{{ task.content }}</td>
            <td>{{ task.date ? $d(Date.parse(task.date), 'short') : '' }}</td>
            <td>
              <div v-if="task.board">
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="synergyDogApp.task.delete.question" data-cy="taskDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-task-heading" v-text="$t('synergyDogApp.task.delete.question', { id: removeId })">
